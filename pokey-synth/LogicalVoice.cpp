...
{    
  ...
  value = value + channel->m_fLFO * conf->lfoToPitch/(127.0/12.0);
  ...
  value = value + m_mod.fValue * conf->modEnvToPitch/(127.0/12.0);
  ...
  switch(conf->distModSrc) {
    // MOD WHEEL
    case TONE_CONFIG::MODSRC_WHEEL: 
      // mod wheel overrides the distortion controller
      value = channel->m_fModWheel;
      break;
    // LFO
    case TONE_CONFIG::MODSRC_LFO:  
      // distortion level from 0..distortion control level
      value = (conf->dist/127.0)/2.0 * (1.0 + channel->m_fLFO);
      break;
    // MOD ENV SCALES DISTORTION
    case TONE_CONFIG::MODSRC_ENV:  
      value = (conf->dist/127.0) * m_mod.fValue;
      break;    
    // JUST DIST DEPTH DIRECTLY
...
/*
  ...
  if(conf->modEnvDest & TONE_CONFIG::TO_DIST) {
    modDepth = conf->ymodEnvDepth/127.0;
    value = (
    (1.0 - modDepth) * value + 
      (modDepth * m_mod.fValue)
      );
...
*/
  ...
  switch(conf->hpfModSrc) {
    // MOD WHEEL
    case TONE_CONFIG::MODSRC_WHEEL: 
      // mod wheel overrides the HPF controller
      value = channel->m_fModWheel;
      break;
    // LFO
    case TONE_CONFIG::MODSRC_LFO:  
      // distortion level from 0..hpf control level
      value = (conf->hpf/127.0)/2.0 * (1.0 + channel->m_fLFO);
      break;
    // MOD ENV SCALES HPF
    case TONE_CONFIG::MODSRC_ENV:  
      value = (conf->hpf/127.0) * m_mod.fValue;
      break;    
    // JUST HPF DEPTH DIRECTLY
    case TONE_CONFIG::MODSRC_NONE:
    default:
