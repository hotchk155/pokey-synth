...
void CLogicalChannel::handleCC(char cc, char value)
{
  switch(cc) {    
    ...
    case CC_MIDIVEL: ccFlag(CF_FULLVELOCITY, value); break;
    case CC_UNISON: ccFlag(CF_UNISON, value); break;
    case CC_ARPMODE: ccFlag(CF_ARPEGGIATE, value); break;
    case CC_MOD:  m_fModWheel = (float)value/127; break;
    case CC_TRIGMIN: m_conf->trigMin = value; break;
    case CC_TRIGMAX: m_conf->trigMax = value; break;
    case CC_TRANSPOSE: m_conf->transpose = value - 64; break;
    case CC_FINETUNE: m_conf->fineTune = ((float)value - 64)/10.0; break;
    case CC_ARPRATE: m_conf->arpPeriod = 127-value; break;
    case CC_ARPCOUNT: m_conf->arpCount = 1 + value/16; break;
    case CC_LFORATE: m_conf->fLFOStep = 1.0/((float)value+1); break;
    case CC_LFOLEVEL: m_conf->fLFOLevel = (float)value/127; break;
    case CC_ENVATTACK: m_conf->fAttackStep = 1.0/((float)value+1); break;
    case CC_ENVRELEASE: m_conf->fReleaseStep = 1.0/((float)value+1); break;
    case CC_PBRANGE: m_conf->pitchBendRange = 12.0 * (value/127.0); break;
    case CC_HPF: m_conf->hpf = 4000.0 * (1.0 -  1.0/((float)value+1));
    case CC_DIST: m_conf->dist = value; break;    
    case CC_DETUNELEVEL: m_conf->detuneLevel = value - 64; break;
    case CC_DETUNEMODE: m_conf->eDetuneMode = ccMapValue(value, DETUNE_MAX); recalc_detune(); break;    
    case CC_MODDEST: m_conf->eModWheelDest = ccMapValue(value, MOD2MAX); break;    
    case CC_LFOMODE: m_conf->eLFOMode = ccMapValue(value, RUN_MAX); break;    
    case CC_LFOWAVE: m_conf->eLFOWave = ccMapValue(value, LFO_MAX); break;    
    case CC_LFODEST: m_conf->eLFODest = ccMapValue(value, LFO2MAX); break;    
    case CC_ENVDEST: m_conf->eEnvelopeDest = ccMapValue(value, ENV2MAX); break;    
